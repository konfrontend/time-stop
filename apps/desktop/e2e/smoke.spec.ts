...
test('launch, run a Timer, quit stops it, relaunch finds the Record', async () => {
  ...
  await expect.poll(() => shellState.windowTitle(first.app)).toBe(productName);
  await dial.click();
  await expect(dial).toHaveAttribute('data-running');
  await expect
    .poll(() => shellState.windowTitle(first.app))
    .toMatch(new RegExp(`^${productName} — \\d{2}:\\d{2}:\\d{2}$`));
  ...
  await expect.poll(() => shellState.windowTitle(second.app)).toBe(productName);
